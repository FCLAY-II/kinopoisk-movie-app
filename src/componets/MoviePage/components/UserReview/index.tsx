...
const UserReview: React.FC<UserReviewProps> = ({movieId, showForm, onCloseForm, onReviewChange}) => {
    const { user } = useAuth();
    const { review, loading, error, saveReview, deleteReview } = useUserReview(movieId);
            ...
            {review && !isEditing && (
                <div className={s.existingReview}>
                    ...
                    <div className={s.reviewDate}>
                        Отзыв от {formatDate(review.createdAt)} пользователь {review.userDisplayName}

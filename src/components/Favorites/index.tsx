  ...
  return (
    <div className={s.favoritesPage}>
          ...
          <div className={s.filtersSection}>
            <div className={s.sortDropdown}>
              ...
              >
                ...
                <ChevronDown
                  size={16}
                  className={showSortMenu ? s.rotated : ""}
                />
              </button>

              {showSortMenu && (
                <div className={s.sortMenu}>
                  {(["addedAt", "rating", "year", "name"] as SortOption[]).map(
                    (option) => (
                      <button
                        key={option}
                        onClick={() => {
      ...
      {!loading && !error && favorites.length === 0 && (
        <div className={s.emptyState}>
          <div className={s.emptyContent}>
            ...
            <Link href="/search-movies" className={s.searchLink}>
              <Search size={20} />
              Найти фильмы
            </Link>
          </div>
      ...
      {!loading && !error && filteredAndSortedFavorites.length > 0 && (
        <div
          className={`${s.moviesGrid} ${viewMode === "list" ? s.listView : ""}`}
        >
